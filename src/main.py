...
def create_monthly_column_sheets(month_sheet,title, init_column,text_column,color_title,color_column):
    ...
    for day_number, day_name in day_to_name.items():
        ...
        cell.value = day_text
        cell.alignment = Alignment(horizontal='center')
        cell.fill = PatternFill(start_color='528EF6', end_color='528EF6', fill_type="solid")  # Color azul pastel
        cell.font = Font(bold=True, name='Calibri')
    ...
    for row_idx, row in enumerate(cell_range, 1):
        for col_idx, cell in enumerate(row, 1):
            if row_idx == 1:  # Borde superior
                cell.border = Border(left=Side(style='thin'),right=Side(style='thin'),top=Side(style='thick'),bottom=Side(style='thin'))
            ...
            if col_idx == len(row):  # Borde derecho
                cell.border = Border(left=Side(style='thin'),right=Side(style='thick'),top=Side(style='thin'),bottom=Side(style='thin'))
